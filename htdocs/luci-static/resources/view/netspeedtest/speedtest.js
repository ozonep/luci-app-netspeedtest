...
return view.extend({
  ...
  poll_status(nodes, res) {
    ...
    if (result_content.length) {
      ...
      if (result_content[0].match(/https?:\S+/)) {
        result_stat.innerHTML =
          "<div style='max-width:500px'><a href='" +
          result_content[0] +
          "' target='_blank'><img src='" +
          result_content[0] +
          ".png" +
          "' style='max-width:100%;max-height:100%;vertical-align:middle'></a></div>";
      }
  ...
  render(res) {
      ...
      if (result_content.length) {
        if (result_content[0] == "Testing") {
          return E("div", { id: "speedtest_result" }, [
            E("span", { style: "color:yellow;font-weight:bold" }, [
              E(
                "img",
                {
        ...
        if (result_content[0] == "Test failed") {
          return E("div", { id: "speedtest_result" }, [
            E("span", { style: "color:red;font-weight:bold" }, [
              _("Test failed."),
            ]),
          ]);
        }
        ...
        return E("div", { id: "speedtest_result" }, [
          E("span", { style: "color:red;font-weight:bold;display:none" }, [
            _("No result."),
          ]),
        ]);
      }
    ...
    o.inputstyle = "apply";
    if (
      result_content.length &&
      result_content[0] == "Testing" &&
      date.getTime() - result_mtime < TestTimeout
    )
      o.readonly = true;
    o.onclick = L.bind(function (ev, section_id) {
      return fs
        .exec("/usr/lib/netspeedtest/speedtest")
        .then((res) => {
    ...
    o.cfgvalue = function (s) {
      if (has_ookla) {
        return E(
          "span",
          { id: "ookla_status", style: "color:green;font-weight:bold" },
          [_("Installed")]
        );
      } else {
        return E(
          "span",
          { id: "ookla_status", style: "color:red;font-weight:bold" },
          [_("Not Installed")]
        );
      }
    };
    ...
    o = s.option(
      form.Flag,
      "proxy_enabled",
      _("Enable proxy for downloader and test")
    );
    o.rmempty = false;
    ...
    o.onclick = function (ev, section_id) {
      ...
      return fs
        .exec_direct("/etc/init.d/netspeedtest", ["download_ookla", arch])
        .catch((e) => {
          ui.addNotification(null, E("p", e.message), "error");
        });
    };
        ...
        function (m, nodes) {
          poll.add(
            L.bind(function () {
              return Promise.all([
                L.resolveDefault(
                  fs.stat("/usr/libexec/netspeedtest/speedtest"),
                  {}
                ),
                L.resolveDefault(fs.read("/var/speedtest_result"), null),
